...
BODIES = [
    (((0, 0), (0, 1), (0, 2), (1, 2)), YELLOW),  # L
    (((0, 0), (1, 0), (2, 0), (1, 1)), ORANGE),  # T
    (((0, 0), (1, 0), (1, 1), (1, 2)), GREEN),  # J
    (((0, 0), (0, 1), (0, 2), (0, 3)), RED),  # I
...
class Piece:
    def __init__(self, body=None, color=None):
        if body is None:
            piece_type, piece_color = self.poll_attacker_ai()
            self.body = BODIES[piece_type][0]  # Extract body coordinates from BODIES list
    ...
    def poll_attacker_ai(self):
        ...
        piece_color = BODIES[piece_type][1]
        return piece_type, piece_color

    def get_available_types(self):
        return {i: 1 for i in range(len(BODIES))}
    def score_types(self, available_types):
        scores = {}
        for type_ in available_types:
            score = 0
            for rotation in range(4):
                rotated_piece = self.rotate_piece(BODIES[type_][0], rotation)  # Rotate the piece
                # Calculate metrics for the rotated piece
                max_height = max(y for _, y in rotated_piece)
                num_holes = self.count_holes(rotated_piece)
                # Update score based on metrics
                score += max_height * 100 + num_holes * 10
            scores[type_] = score
        return scores

    def rotate_piece(self, piece, rotation):
        # Rotate the piece coordinates
        if rotation == 0:
            return piece
        elif rotation == 1:
    ...
    def count_holes(self, piece):
        ...
        for x, y in piece:
            if 0 <= x < width and 0 <= y < height:  # Check if coordinates are within grid bounds
                grid[y][x] = 1  # Mark filled cells in the grid
        
        # Iterate over each column
        for x in range(width):
            # Flag to track if we've encountered a filled cell in this column
            found_block = False
            for y in range(height):
                if 0 <= y < height and 0 <= x < width:  # Check if coordinates are within grid bounds
                    if grid[y][x] == 1:
                        found_block = True  # Found a filled cell
                    elif grid[y][x] == 0 and found_block:
                        num_holes += 1  # Found a hole below a filled cell
                else:
                    break  # Break loop if coordinates are out of bounds
            # Reset the flag for the next column
            found_block = False
        return num_holes
...
class Game1:
    ...
    def run(self):
        ...
        pygame.quit()
        sys.exit()
    def draw_text_with_highlight(self, text, x, y, fontSize=int(28*1.5), highlight_color=(0, 0, 50)):
