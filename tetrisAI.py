...
class Tetris:
    ...
    def calculate_row_fill_score(self, grid):
        row_fill_score = 0
        ...
        for col in range(num_columns):
            for row in range(len(grid)):
                if grid[row][col] != 0:
                    column_heights[col] = len(grid) - row
                    break

        # Calculate the standard deviation of column heights
        mean_height = sum(column_heights) / num_columns
        std_deviation = (sum((height - mean_height) ** 2 for height in column_heights) / num_columns) ** 0.5

        # Lower standard deviation implies more even distribution of heights, hence better row filling
        row_fill_score = 1 / (1 + std_deviation)

        return row_fill_score
        

    def calculate_score(self, holes, max_height, lines_cleared, wells, grid=None):
        ...
        line_clear_bonus = 1000  # Bonus for each cleared line
        position_penalty_weight = 20  # Penalty for position difference
        ...
        # Calculate row fill heuristic
        if grid:
            row_fill_score = self.calculate_row_fill_score(grid)
            score += row_fill_weight * row_fill_score

        # Bonus for each line cleared
        score += lines_cleared * line_clear_bonus

        # Add penalty for position difference if max height and holes are the same
        
        #score -= position_penalty_weight * self.calculate_position_difference(grid)

        return score
        
    def calculate_position_difference(self, grid):
        if grid is None or not any(grid):
            return 0
        
        original_height = len(grid)
        final_height = 0
        for row in reversed(grid):  # Iterate from bottom to top
            if any(row):
                final_height = len(grid) - grid.index(row)
                break
        position_difference = original_height - final_height
        return position_difference

        
    def calculate_wells(self, grid):
        wells = 0
        num_columns = len(grid[0])

        for col in range(num_columns):
            for row in range(len(grid)):
                if grid[row][col] == 0:  # Found a potential well
                    if col == 0:  # Left edge
                        if grid[row][col + 1] != 0:
                            wells += 1
                        ...
                        if grid[row][col - 1] != 0 and grid[row][col + 1] != 0:
                            wells += 1
                        break  # Exit loop once a potential well is found

        return wells
        
    from collections import deque

    def get_best_move(self, grid_copy, current_piece, piece_x, piece_y, max_height, max_depth=20):
        ...
        # Calculate initial height and number of holes
        max_height = sum(1 for row in grid_copy if any(row))
        holes = self.count_holes_in_range(max_height, grid=grid_copy)
        lines_cleared = self.get_clear_lines(grid_copy)

        # Define a queue for BFS
        queue = deque([(original_grid, current_piece, piece_x, piece_y, 0)])

        # Perform BFS
        while queue:
            grid, piece, x, y, depth = queue.popleft()
                ...
                if move == "ROTATE":
                    for rotation in range(4):  # Assuming 4 possible rotations
                        ...
                        if self.is_valid_position(rotated_piece_copy, piece_x_copy, piece_y_copy, grid_copy):
                            ...
                            new_lines_cleared = self.get_clear_lines(grid_copy)
                            wells = self.calculate_wells(grid_copy)
                            score = self.calculate_score(new_holes, max_height, new_lines_cleared, wells)

                            if score < best_score:
                                best_score = score
                                best_move = move

                            queue.append((grid_copy, rotated_piece_copy, piece_x_copy, piece_y_copy, depth + 1))
                    ...
                    while True:
                        ...
                        new_max_height = sum(1 for row in grid_copy if any(row))
                        new_holes = self.count_holes_in_range(new_max_height, grid=grid_copy)
                        new_lines_cleared = self.get_clear_lines(grid_copy)
                        wells = self.calculate_wells(grid_copy)
                        if new_lines_cleared > lines_cleared:
                            return move

                        score = self.calculate_score(new_holes, new_max_height, new_lines_cleared,wells)
                        if score < best_score:
                            best_score = score
                            best_move = move
