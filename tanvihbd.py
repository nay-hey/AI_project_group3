...
class Draw:
    ...
    def draw_main_screen(self):
        ...
        self.draw_text_with_highlight("PRESS  R  :  HUMAN", (self.boardWidth // 2 - 0.7) * BLOCK_SIZE,
...
class Tetris:
    ...
    def generate_ai_piece(self):
        # Implement Minimax algorithm to generate piece
        return self.ai_minimax_decision()
    ...
    def drop_piece_hard(self):
        while not self.check_collision(self.current_piece, self.piece_x, self.piece_y + 1):
            self.piece_y += 1
        self.merge_piece()
        self.current_piece = self.next_piece()  # Set current piece to
  # Generate new random piece for next piece    
        self.piece_x = self.board.GRID_WIDTH // 2 - len(self.current_piece[0]) // 2
        self.piece_y = 0
        ...
        for y in range(len(self.current_piece)):
            for x in range(len(self.current_piece[y])):
                if self.current_piece[y][x]:
                    target_y = y + self.piece_y
                    target_x = x + self.piece_x
                    if 0 <= target_y < self.board.GRID_HEIGHT and 0 <= target_x < self.board.GRID_WIDTH:
                        self.grid[target_y][target_x] = self.current_piece[y][x]
                    else:
                        print("Attempted to merge piece out of grid bounds:", target_y, target_x)

    def ai_minimax_decision(self):
        ...
        for x in range(self.board.GRID_WIDTH):
            for rotation in range(4):
                ...
                for _ in range(rotation):
                    piece = [list(reversed(row)) for row in zip(*piece)]
                y = 0
                while not self.check_collision(piece, x, y):
                    y += 1
                moves.append((piece, x, y - 1))
        return moves
    ...
    def handle_ai_input(self):
        ai_piece, ai_x, ai_y = self.generate_ai_piece()
        self.current_piece = ai_piece
        self.piece_x = ai_x
        self.piece_y = ai_y
...
if __name__ == "__main__":
    ...
    # Ask user for AI mode selection
    ai_mode = simpledialog.askstring(title="AI Mode Selection", prompt="Enter AI mode (Minimax or Random): ")
    pygame.init()
